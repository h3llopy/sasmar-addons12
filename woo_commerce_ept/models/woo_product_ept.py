...
class woo_product_template_ept(models.Model):
    ...
    def auto_update_stock_ept(self,ctx={}):
        ...
        #Add by Haresh Mori on data 2/10/2019 We have checked the instance state while we perform the operation from schedule activity. 
        if instance.state != 'confirmed':
            transaction_log_obj.create({'message':"While the update stock, Instance(%s) is not in confirm state.\n Please set schedule activity False, schedule activity name like '%s : Update Woo Stock' "%(instance.name,instance.name),
             'mismatch_details':True,
             'type':'product',
             'woo_instance_id':instance.id
            })

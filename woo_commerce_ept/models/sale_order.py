...
#             })
    ...
    def get_woo_order_vals(self,result,workflow,invoice_address,instance,partner,shipping_address,pricelist_id,fiscal_position,payment_term,payment_gateway):
            ...
            if instance.woo_version == 'old':
                ...
                #Converting time into UTC timezone @author: Priya Pal 8th Jan 2020
                created_at = str(pytz.timezone(instance.store_timezone[:19]).localize(parser.parse(date_created), is_dst=None).astimezone(pytz.utc))[:19] if instance.store_timezone else date_created
    ...
    def auto_import_woo_sale_order_ept(self,ctx={}):
        ...
        #Add by Haresh Mori on data 2/10/2019 We have checked the instance state while we perform the auto import order operation from schedule activity. 
        if instance.state != 'confirmed':
            transaction_log_obj.create({'message':"While the auto import order, Instance(%s) is not in confirm state.\n Please set schedule activity False, schedule activity name like '%s : Import Woo Orders' "%(instance.name,instance.name),
             'mismatch_details':True,
             'type':'product',
             'woo_instance_id':instance.id
            })
    ...
    def import_new_woo_orders(self,instance=False,before_date=False,after_date=False,is_cron=True):
        ...
        for instance in instances:
            ...
            if instance.store_timezone:
                ...
                pytz.timezone(instance.store_timezone[:19] or 'UTC')).strftime('%Y-%m-%d %H:%M:%S'), "%Y-%m-%d %H:%M:%S") \
...
#                     sale_order.write({'fiscal_position_id':False})
    ...
    def auto_update_woo_order_status_ept(self,ctx={}):
        ...
        woo_instance_id = ctx.get('woo_instance_id',False)
        instance = self.env['woo.instance.ept'].browse(woo_instance_id)        
        #Add by Haresh Mori on data 2/10/2019 We have checked the instance state while we perform the operation auto update stock from schedule activity. 
        if instance.state != 'confirmed':
            transaction_log_obj.create({'message':"While the update order status, Instance(%s) is not in confirm state.\n Please set schedule activity False, schedule activity name like '%s : Update Woo Order Status' "%(instance.name,instance.name),
             'mismatch_details':True,
             'type':'product',
             'woo_instance_id':instance.id
            })
